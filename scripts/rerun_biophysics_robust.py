"""
Re-run the module 5 biophysics analysis on gsi_robust instead of raw GSI.

Raw GSI has outliers up to 747 from near-zero shuffle_mean denominators.
gsi_robust stabilizes the denominator:
  gsi_robust = shuffle_std / max(|shuffle_mean|, shuffle_std * 0.1)

With raw GSI the human datasets came out at negative R2 (agarwal -9.56, klein
-19.14, inoue -0.57).
"""
...
def main():
    ...
    for ds_name in DATASETS:
        ...
        # old results, for the comparison column
    ...
    print(f"\n{'='*60}")
    print("biophysics R2, raw GSI vs gsi_robust")

#!/usr/bin/env python3
"""
What do the foundation models actually key on?

Decompose their predictions into interpretable features: k-mer composition
(1-mer to 6-mer), GC and dinucleotide statistics, DNA shape (MGW, Roll, ProT,
HelT), and spacer-only versus motif features, then report how much variance
each block explains.
"""
...
def compute_shape_features(sequence):
    """Dinucleotide proxies for DNA shape; real MGW/Roll/ProT/HelT needs lookup tables."""
    # dinucleotide step parameters, simplified from Rohs et al.
    roll_values = {
...
def extract_all_features(sequence):
    ...
    # full k-mer is too slow, dinucs + trinucs only
    for k in [3]:
...
def run_feature_decomposition(dataset_name, model_name='dnabert2', n_samples=500, seed=42):
    ...
    # nested fits, one feature block at a time

"""
Attention-based grammar extraction.

For the transformer models (DNABERT-2, NT v2-500M), look at motif-to-motif
attention and try to find heads that encode arrangement.
"""
...
def extract_attention_patterns(
    ...
    """
    Motif-to-motif attention per (layer, head), scored against a background of
    random positions. Transformer models only.
    """
                ...
                for mi, (name_i, start_i, end_i) in enumerate(motif_token_ranges):
                    for mj, (name_j, start_j, end_j) in enumerate(motif_token_ranges):
                        ...
                        if end_i <= seq_len and end_j <= seq_len:
                            ...
                            # background: attention to random positions
                            bg_attn = attn.mean()
...
def identify_grammar_heads(
    ...
    """
    Heads where motif-to-motif attention is enriched over background and the
    enrichment tracks grammar sensitivity.
    """
...
def compare_attention_original_vs_shuffled(
    ...
    """Which heads change most between original and shuffled arrangements?"""
    results = []
    ...
    for orig_seq, shuf_seq, annot in zip(original_seqs, shuffled_seqs, motif_annotations):
        ...
        # same shape expected, sequences are the same length
...
def _get_dnabert2_attention(model, sequence: str) -> Optional[np.ndarray]:
    ...
    with torch.no_grad():
        # output_attentions may work with the custom BertModel
        try:
            ...
            # some custom models return a tuple; index 2 is usually attentions
        ...
        # fall back to hooks on the self-attention modules to grab QK attention
        attentions = []
        handles = []
        ...
        for layer in model.model.encoder.layer:
            # hook the self-attention sub-module, not the attention wrapper
        ...
        for layer_attns in attentions:
            if layer_attns:
                attn_array.append(layer_attns[0][0])  # drop batch dim
...
def _map_motifs_to_tokens(model, sequence: str, motifs: List[Dict]) -> List[Tuple[str, int, int]]:
    ...
    # approximate; exact mapping depends on the tokenizer
    if model.name == 'dnabert2':
        # DNABERT-2 is BPE, roughly 6 chars per token
        char_per_token = 6
    elif model.name == 'nt':
        # NT is 6-mers
        char_per_token = 6
